...
class moea_NSGA2_templet(ea.Algorithm):
    #输入的是ea.Algorithm类//属性包括problem 和 种群对象 population  等等
    #init中接收的是problem和population
    """
...
参考文献:
    ...
    def terminated(self, population): # 判断是否终止进化
        if self.currentGen < self.MAXGEN:
            ...
            if self.drawing == 2:
                self.ax = ea.moeaplot(population.ObjV, False, self.ax, self.currentGen) # 绘制动态图  False表示不保存图片  self.ax不为None是可以实现
                #动态绘制
